...
@auth
        ...
        <body class="bg-base-100">
            <div class="flex h-screen overflow-hidden">
                ...
                <div class="flex flex-col flex-1 overflow-hidden">
                    ...
                    <section class="w-full min-h-screen mt-5 space-y-6">
        @if (session('success'))
            <div id="success-message"
                ...
                <button onclick="closeSuccessMessage()" class="flex-shrink-0 text-white hover:text-green-100 transition-colors">
        ...
        <div class="bg-white rounded-2xl shadow-2xl overflow-hidden transform transition-all duration-300 hover:shadow-3xl">
            <!-- Header -->
            <div class="relative bg-gradient-to-r from-blue-900 via-blue-800 to-blue-900 px-8 py-10 overflow-hidden">
            ...
            <!-- Profile Form -->
            <form action="{{ route('department.profile.update') }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-8">
                @csrf
                @method('PUT')

                <!-- Profile Photo -->
                <div class="flex flex-col md:flex-row items-center md:space-x-8 bg-gradient-to-r from-blue-50 to-yellow-50 p-6 rounded-xl border-2 border-blue-100 hover:border-yellow-400 transition-all duration-300">
                    <div class="relative group mb-4 md:mb-0">
                        <div class="group w-32 h-32 rounded-full overflow-hidden bg-blue-800 flex items-center justify-center ring-4 ring-yellow-400 transition-all duration-300 hover:ring-8 hover:scale-105">
                            @php
    $user = Auth::user();
    ...
    $initials = strtoupper(substr($firstName, 0, 2));
                            ...
                            <span id="preview-placeholder" class="text-4xl font-bold text-yellow-400 {{ $photo ? 'hidden' : '' }}">
                                {{ $initials }}
                            </span>
                        </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <!-- Department Name -->
                    <div>
                        <label class="block text-sm font-semibold text-blue-900 mb-2">Department Name</label>
                        <input type="text" name="dept_name" value="{{ old('dept_name', Auth::user()->dept_name) }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="employee_name" value="{{ old('employee_name', Auth::user()->employee_name) }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="employee_id" value="{{ old('employee_id', Auth::user()->employee_id) }}"
                            class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg bg-gray-100 cursor-not-allowed"
                    ...
                    <!-- Employee Status -->
                    <div>
                        <label class="block text-sm font-semibold text-blue-900 mb-2">Employee Status</label>
                        <input type="text" name="status" value="{{ old('employee_status', Auth::user()->status) }}"
                            class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg bg-gray-100 cursor-not-allowed"
                    ...
                    <div>
                        ...
                        <input readonly type="text" name="role" value="{{ old('role', Auth::user()->role) }}"
                            class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg bg-gray-100 cursor-not-allowed"
                ...
                <div class="pt-6 border-t border-gray-200">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label class="block text-sm font-semibold text-blue-900 mb-2">New Password</label>
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-blue-900 mb-2">Confirm Password</label>
            ...
            <div class="bg-white rounded-2xl shadow-2xl overflow-hidden transform transition-all duration-300 hover:shadow-3xl">
                <div class="bg-gradient-to-r from-blue-900 to-blue-800 px-6 py-4">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="px-3 py-1 bg-yellow-400 text-blue-900 rounded-full text-sm font-semibold">
                                    ...
                                    @foreach($loginLogs as $log)
                                        <tr class="hover:bg-blue-50 transition-colors duration-200">
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                ...
                                                @else
                                                    <span class="px-2 py-1 bg-yellow-100 text-yellow-800 text-xs rounded-full">ATTEMPT</span>
                                                @endif
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="text-sm text-gray-900">{{ ucfirst($log->log_type) ?? 'Login' }}</div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="text-sm text-gray-900">{{ $log->attempt_count }}/5</div>
                                                @if($log->failure_reason)
                                                    <div class="text-xs text-gray-500">{{ $log->failure_reason }}</div>
                                                @endif
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="text-sm text-gray-900">{{ \Carbon\Carbon::parse($log->date)->format('M d, Y') }}
                                                </div>
                                                <div class="text-xs text-gray-500">{{ \Carbon\Carbon::parse($log->date)->format('h:i A') }}
                                                </div>
                                            </td>
                                        </tr>
                                    @endforeach
                ...
                <div class="p-6">
                                        ...
                                        <tr class="hover:bg-blue-50 transition-colors duration-200">
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                @if($audit->action == 'CREATE')
                                                    <span class="px-2 py-1 bg-green-100 text-green-800 text-xs rounded-full">CREATE</span>
                                                @elseif($audit->action == 'UPDATE')
                                                    <span class="px-2 py-1 bg-yellow-100 text-yellow-800 text-xs rounded-full">UPDATE</span>
                                                @elseif($audit->action == 'DELETE')
                                                    <span class="px-2 py-1 bg-red-100 text-red-800 text-xs rounded-full">DELETE</span>
                                                @else
                                                    <span class="px-2 py-1 bg-blue-100 text-blue-800 text-xs rounded-full">{{ $audit->action }}</span>
                                                @endif
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="text-sm font-medium text-gray-900">{{ $audit->modules_cover }}</div>
                                            </td>
                                            <td class="px-6 py-4">
                                                <div class="text-sm text-gray-900">{{ $audit->activity }}</div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="text-sm text-gray-900">{{ \Carbon\Carbon::parse($audit->date)->format('M d, Y') }}</div>
                    ...
                    @else
                        <div class="text-center py-12">
                            ...
                            <p class="text-gray-500">No audit trail records found</p>
                        </div>
                    @endif
                </div>
            </div>
        ...
        <div class="mt-8 bg-blue-900 rounded-xl p-6 shadow-lg">
            <div class="flex items-start space-x-4">
                ...
                <div>
                    ...
                    <p class="text-blue-100 text-sm">Your information is encrypted and securely stored. All activities are logged for security purposes. We never share your personal details with third parties.</p>
                </div>
            </div>
        </div>
