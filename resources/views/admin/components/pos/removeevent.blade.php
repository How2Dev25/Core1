<dialog id="removeevent_{{ $reserveevent->eventposID }}" class="modal">
    <div class="modal-box bg-white shadow-2xl border border-gray-200 rounded-2xl max-w-md mx-auto">
        <div class="flex items-center gap-4 mb-6">
            ...
            <h3 class="font-bold text-xl text-red-600">Remove Event?</h3>
        </div>

        <p class="py-4 text-gray-600 leading-relaxed">
            Are you sure you want to remove this event from POS? This action cannot be undone.
        </p>

        <form method="POST" action="/removeeventpos/{{ $reserveevent->eventposID }}" class="flex gap-4 mt-6">
            @csrf
            @method('DELETE')
            <button type="button" class="btn btn-outline btn-neutral flex-1 hover:bg-gray-100 transition-colors"
                onclick="document.getElementById('removeevent_{{ $reserveevent->eventposID }}').close()">Cancel</button>
            <button type="submit"
