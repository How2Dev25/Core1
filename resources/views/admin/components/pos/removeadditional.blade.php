<dialog id="removeinventory_{{ $additional->inventoryposID }}" class="modal">
    <div class="modal-box bg-white shadow-2xl border border-gray-200 rounded-2xl max-w-md mx-auto">
        ...
        <p class="py-4 text-gray-600 leading-relaxed">
            Are you sure you want to remove this additional {{ $additional->core1_inventory_name }}  from POS? This action cannot be undone.
        </p>

        <form method="POST" action="/removeadditional/{{ $additional->inventoryposID }}" class="flex gap-4 mt-6">
            @csrf
            @method('DELETE')
            <button type="button" class="btn btn-outline btn-neutral flex-1 hover:bg-gray-100 transition-colors"
                onclick="document.getElementById('removeinventory_{{ $additional->inventoryposID }}').close()">Cancel</button>
            <button type="submit"
