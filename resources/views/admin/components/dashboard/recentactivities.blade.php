    ...
    @forelse ($sessions as $session)
        ...
        <div
            ...
            <div class="flex items-center gap-3 w-full sm:w-auto relative group/tooltip">
                ...
                <!-- Tooltip Modal -->
                <div
                    class="absolute z-50 left-14 top-1/2 transform -translate-y-1/2 hidden group-hover/tooltip:block bg-white shadow-xl rounded-xl border border-gray-200 w-64 p-4 animate-fadeIn">
                    <div class="flex flex-col items-center text-center">
                        @if ($photoPath)
                            <img src="{{ asset($photoPath) }}" class="w-16 h-16 rounded-full object-cover mb-3 shadow">
                        @else
                            <div
                                class="w-16 h-16 bg-gradient-to-br {{ $gradient }} rounded-full flex items-center justify-center text-white font-bold shadow mb-3">
                                {{ strtoupper(substr($session->employee_name, 0, 2)) }}
                            </div>
                        @endif

                        <h4 class="font-semibold text-gray-900">{{ $session->employee_name }}</h4>
                        <p class="text-sm text-gray-500 mb-1">{{ ucfirst($session->role ?? 'User') }}</p>
                        <p class="text-xs text-gray-400">{{ $session->email ?? 'No email provided' }}</p>
                        <span
                            class="mt-2 inline-block text-xs font-medium text-green-600 bg-green-100 px-2 py-1 rounded-full">
