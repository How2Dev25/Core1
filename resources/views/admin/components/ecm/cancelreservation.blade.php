<dialog id="cancel_event_{{$reservation->eventbookingID}}" class="modal">
    <div class="modal-box max-w-md">
        ...
        <form method="POST" action="/cancelbookingevent/{{$reservation->eventbookingID}}">
            @csrf
            @method('PUT')
            <div class="modal-action">
                <button type="button" onclick="cancel_event_{{$reservation->eventbookingID}}.close()"
                    class="btn btn-ghost">
                    Cancel
                </button>
                <button type="submit" class="btn btn-info">
                    <i class="fa-solid fa-circle-xmark w-4 h-4 mr-1"></i>
                    Cancel Reservation
                </button>
            </div>
        </form>
