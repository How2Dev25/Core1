        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
                <div class="p-3 bg-white/20 backdrop-blur-sm rounded-xl shadow-lg">
                    <i class="fa-solid fa-calendar-days text-2xl text-yellow-400"></i>
                </div>
                <div>
                    <h3 class="text-xl font-bold text-white">Book An Event</h3>
                    <p class="text-sm text-white/80">Reserve your special moment</p>
                </div>
            </div>
            <div class="hidden sm:flex items-center gap-2">
                <button id="prevBtn" class="p-2 bg-white/20 hover:bg-white/30 rounded-lg transition-colors">
                    <i class="fas fa-chevron-left text-white"></i>
                </button>
                <button id="nextBtn" class="p-2 bg-white/20 hover:bg-white/30 rounded-lg transition-colors">
        ...
        @if($events->isNotEmpty())
            <div class="relative">
                    ...
                    <div id="carouselSlides" class="flex transition-transform duration-500 ease-in-out">
                        @foreach($events as $index => $event)
                            <div class="flex-shrink-0 w-full px-2">
                                <a href="#" class="group block relative rounded-xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300">
                                    <!-- Image -->
                                    <div class="relative h-80 overflow-hidden">
                                        ...
                                        <div class="absolute bottom-0 left-0 right-0 p-6">
                                            <div class="flex items-start justify-between mb-3">
                                                <div class="flex-1">
                                                    <span class="inline-block px-3 py-1 bg-yellow-400 text-blue-900 rounded-full text-xs font-bold mb-3">
                                                        FEATURED EVENT
                                                    </span>
                                            ...
                                            <div class="flex flex-wrap gap-2 mb-4">
                                                @if(isset($event->eventtype_capacity))
                                                    <span class="flex items-center gap-1 px-3 py-1.5 bg-white/20 backdrop-blur-sm text-white rounded-lg text-xs">
                                            ...
                                            <!-- CTA Button -->
                                            <button class="w-full py-3 bg-yellow-400 hover:bg-yellow-500 text-blue-900 font-bold rounded-xl transition-all duration-300 flex items-center justify-center gap-2 group-hover:scale-105">
                ...
                <div class="flex sm:hidden justify-center mt-4 gap-3">
                    <button id="prevBtnMobile" class="p-3 bg-blue-900 hover:bg-blue-800 text-yellow-400 rounded-lg transition-colors shadow-lg">
                        <i class="fas fa-chevron-left"></i>
                    </button>
                    <button id="nextBtnMobile" class="p-3 bg-blue-900 hover:bg-blue-800 text-yellow-400 rounded-lg transition-colors shadow-lg">
                ...
                <div class="flex justify-center mt-6 gap-2">
                    @foreach($events as $index => $event)
                        <button class="carousel-dot w-2.5 h-2.5 rounded-full bg-gray-300 hover:bg-gray-400 transition-all duration-300" 
