<dialog id="loyaltyModal" class="modal">
    <div class="modal-box bg-white rounded-2xl p-6 border-2 border-blue-900/10">
        ...
        <div class="bg-gradient-to-r from-blue-900/5 to-orange-400/5 rounded-xl p-4 mb-4 border border-blue-900/10">
            ...
            <p class="text-2xl font-bold text-blue-900" id="availablePoints">{{ $myloyaltypoints ?? 0 }}</p>
        </div>
...
<script>
    ...
    document.getElementById('applyLoyaltyBtn').addEventListener('click', () => {
        ...
        const vat = parseFloat(document.getElementById('hiddenVat').value) || 0;
        const serviceFee = parseFloat(document.getElementById('hiddenServiceFee').value) || 0;
    ...
    // Reset loyalty discount when modal is closed without applying
    document.getElementById('loyaltyModal').addEventListener('close', (event) => {
        // Only reset if discount wasn't applied (check if modal closed via cancel button or backdrop)
        if (event.target.returnValue === '') {
            const selected = document.querySelector('input[name="selectedRule"]:checked');
            if (selected && loyaltyDiscountValue === 0) {
                selected.checked = false;
            }
        }
    });
