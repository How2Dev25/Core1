<dialog id="deleteordermodal-{{$carts->orderID}}" class="modal">
    <div class="modal-box bg-white rounded-xl p-6 max-w-md text-center">
        ...
        <form action="/cancelorder/{{$carts->orderID}}" method="POST" class="mt-6 flex justify-center gap-3">
            @csrf
            @method('DELETE')
            <button onclick="document.getElementById('deleteordermodal-{{$carts->orderID}}').close()" type="button"
