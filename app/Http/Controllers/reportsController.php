...
/**
 * Send OTP for billing report access
 */
public function sendBillingReportOTP(Request $request)
{
    try {
        $user = Auth::user();
        ...
        // Store OTP in cache
        Cache::put('billing_report_otp_' . $email, [
            'code' => $otp,
            'created_at' => now(),
            'attempts' => 0
        ], now()->addMinutes(10));
        
        Cache::put('billing_report_attempts_' . $email, 0, now()->addMinutes(30));

        // Send OTP email
        $emailSent = $this->sendBillingReportOTPEmail($email, $otp);
        ...
        Log::info('Billing Report OTP Generated', [
            'email' => $email,
            'otp' => $otp
        ]);
        ...
        Log::error('Billing Report OTP Send Error: ' . $e->getMessage());
        return response()->json([
            'success' => false,
            'message' => 'An error occurred while sending verification code.'
        ], 500);
...
/**
 * Verify OTP for billing report access
 */
public function verifyBillingReportOTP(Request $request)
{
    $request->validate([
        'otp' => 'required|string|size:6',
        'email' => 'required|email'
    ...
    try {
        ...
        $cachedData = Cache::get('billing_report_otp_' . $email);
        $attempts = Cache::get('billing_report_attempts_' . $email, 0);
        ...
        if ($cachedData['code'] === $request->otp) {
            $createdAt = Carbon::parse($cachedData['created_at']);
            if ($createdAt->diffInMinutes(now()) > 10) {
                Cache::forget('billing_report_otp_' . $email);
                return response()->json([
                    'success' => false,
                    'message' => 'Verification code has expired. Please request a new one.',
                    'code' => 'OTP_EXPIRED'
            ...
            // Clear OTP and reset attempts
            Cache::forget('billing_report_otp_' . $email);
            Cache::forget('billing_report_attempts_' . $email);
            ...
            // Store verification in session
            session(['billing_report_verified_' . $email => true]);

            Log::info('Billing Report OTP Verified Successfully', [
                'email' => $email,
                'ip' => $request->ip()
            ]);
            ...
            // Increment attempts
            $newAttempts = $attempts + 1;
            Cache::put('billing_report_attempts_' . $email, $newAttempts, now()->addMinutes(30));
            
            // Update attempts in OTP data
            if ($cachedData) {
                $cachedData['attempts'] = $newAttempts;
                Cache::put('billing_report_otp_' . $email, $cachedData, now()->addMinutes(10));
            }
        ...
        Log::error('Billing Report OTP Verify Error: ' . $e->getMessage());
        return response()->json([
            'success' => false,
            'message' => 'An error occurred while verifying the code.'
        ], 500);
...
/**
 * Resend OTP for billing report access
 */
public function resendBillingReportOTP(Request $request)
{
    try {
        $user = Auth::user();
        ...
        // Rate limiting
        $lastSent = Cache::get('billing_report_last_sent_' . $email);
        if ($lastSent && Carbon::parse($lastSent)->diffInSeconds(now()) < 30) {
            $waitTime = 30 - Carbon::parse($lastSent)->diffInSeconds(now());
            return response()->json([
                'success' => false,
        ...
        Cache::put('billing_report_attempts_' . $email, 0, now()->addMinutes(30));
        Cache::put('billing_report_last_sent_' . $email, now(), now()->addSeconds(30));

        $emailSent = $this->sendBillingReportOTPEmail($email, $otp, true);
        ...
        Log::error('Billing Report OTP Resend Error: ' . $e->getMessage());
        return response()->json([
            'success' => false,
            'message' => 'An error occurred while resending the code.'
        ], 500);
...
/**
 * Send billing report OTP email
 */
private function sendBillingReportOTPEmail($email, $otp, $isResend = false)
{
    $mail = new PHPMailer(true);
    ...
    try {
        ...
        $mail->isHTML(true);
        $mail->Subject = $isResend ? 
            'New Verification Code - Billing Report Access - Soliera Hotel' : 
            'Billing Report Access - Verification Code - Soliera Hotel';
        
        $mail->Body = $this->generateBillingOTPEmailBody($otp, $email, $isResend);
        $mail->AltBody = "Your verification code is: {$otp}. Valid for 10 minutes.";
        ...
        Log::error("Billing Report OTP Email Error: " . $mail->ErrorInfo);
        return false;
    }
}

/**
 * Generate billing OTP email body
 */
private function generateBillingOTPEmailBody($otp, $email, $isResend = false)
{
    $action = $isResend ? 'New Verification Code' : 'Verify Your Access';
    $message = $isResend ? 
        'You requested a new verification code for the billing report.' : 
        'Use the verification code below to access the billing report.';
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Billing Report Verification - Soliera Hotel</title>
</head>
<body style="margin:0; padding:0; font-family: Arial, sans-serif; background-color:#f4f7fc;">
    <div style="max-width:600px; margin:20px auto; background:#ffffff; border-radius:10px; overflow:hidden; box-shadow:0 5px 15px rgba(0,31,84,0.15);">
        <div style="background:#001f54; padding:30px; text-align:center;">
            <img src="cid:hotelLogo" alt="Soliera Hotel" style="width:80px; height:80px; border-radius:50%; border:3px solid #F7B32B; margin-bottom:10px;">
            <h1 style="color:#F7B32B; margin:0; font-size:28px;">SOLIERA HOTEL</h1>
            <p style="color:#ffffff; margin:5px 0 0 0;">Billing Report Access</p>
        </div>
        <div style="padding:30px;">
            <div style="text-align:center; margin-bottom:25px;">
                <span style="background:#F7B32B; color:#001f54; padding:8px 20px; border-radius:20px; font-weight:bold;">🔐 {$action}</span>
